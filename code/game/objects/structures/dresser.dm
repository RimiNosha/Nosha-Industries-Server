...
/obj/structure/dresser/attack_hand(mob/user, list/modifiers)
	...
	switch(choice)
		if("Underwear")
			var/new_undies = tgui_input_list(user, "Select your underwear", "Changing", SSaccessories.get_accessory_list(/datum/sprite_accessory/underwear))
			if(new_undies)
		...
		if("Undershirt")
			var/new_undershirt = tgui_input_list(user, "Select your undershirt", "Changing", SSaccessories.get_accessory_list(/datum/sprite_accessory/undershirt))
			if(new_undershirt)
				dressing_human.undershirt = new_undershirt
		if("Socks")
			var/new_socks = tgui_input_list(user, "Select your socks", "Changing", SSaccessories.get_accessory_list(/datum/sprite_accessory/socks))
			if(new_socks)
