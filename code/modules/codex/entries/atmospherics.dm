/datum/codex_entry/atmos_pipe
	...
	use_typesof = TRUE
	mechanics_text = list(
		"All pipes can be connected or disconnected with a wrench. More pipes can be obtained from a pipe dispenser. ",
...
/datum/codex_entry/atmos_valve
	...
	use_typesof = TRUE
	mechanics_text = list("Click this to turn the valve.  If red, the pipes on each end are seperated.  Otherwise, they are connected.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_pump
	...
	mechanics_text = list("This moves gas from one pipe to another.  A higher target pressure demands more energy.  The side with the red end is the output.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_vent_pump
	...
	mechanics_text = list("This pumps the contents of the attached pipe out into the atmosphere, if needed.")
	controls_text = list("Multitool - Toggle Low-Power mode.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_freezer
	...
	mechanics_text = list("Adjusts the temperature of gas flowing through it.  It uses massive amounts of electricity while on. It can be upgraded by replacing the capacitors, manipulators, and matter bins.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_vent_scrubber
	...
	mechanics_text = list("This filters the atmosphere of harmful gas.  Filtered gas goes to the pipes connected to it, typically a scrubber pipe. It can be controlled from an Air Alarm.  Has a mode to siphon much faster, using much more power in the process.")
	controls_text = list("Multitool - Toggle Low-Power mode.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_canister
	...
	use_typesof = TRUE
	mechanics_text = list("The canister can be connected to a connector port with a wrench.  Tanks of gas (the kind you can hold in your hand) can be filled by the canister, by using the tank on the canister, increasing the release pressure, then opening the valve until it is full, and then close it.  *DO NOT* remove the tank until the valve is closed.  A gas analyzer can be used to check the contents of the canister.")
	antag_text = list("Canisters can be damaged, spilling their contents into the air, or you can just leave the release valve open.")
	disambiguator = "atmospherics"
...
/datum/codex_entry/atmos_meter
	...
	mechanics_text = list("Measures the volume and temperature of the pipe under the meter.")
	disambiguator = "atmospherics"

/datum/codex_entry/transfer_valve
	name = "Tank Transfer Valve"
	associated_paths = list(/obj/item/transfer_valve)
	mechanics_text = list("This machine is used to merge the contents of two different gas tanks. Plug the tanks into the transfer, then open the valve to mix them together. You can also attach various assembly devices to trigger this process.")
	antag_text = list("With a tank of hot hydrogen and cold oxygen, this benign little atmospheric device becomes an incredibly deadly bomb. You don't want to be anywhere near it when it goes off.")
